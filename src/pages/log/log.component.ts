...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, InputTextModule, MessageModule, TableModule, InfiniteScrollModule],
  template: `
    ...
    <div *ngIf="!errorMessage" class="flex align-items-center flex-wrap card-container mt-2 mb-2">
      <p-message severity="info" *ngIf="!pagedmessage;else endOfLogs" text="Loading messages..." ></p-message>
    </div>
    <ng-template #endOfLogs>
      <p-message *ngIf="pagedmessage!.finalPage && totalMessages !== 0;else loadMore" severity="info"  text="End of logs."></p-message>
    </ng-template>
    <ng-template #loadMore>
      <p-message *ngIf="totalMessages !== 0" severity="info"  text="Keep scrolling to load more messages."></p-message>
    </ng-template>
  `,
  styles: [
  ]
...
export class LogComponent {
  ...
  makeInfoMessage(): void {
    if (this.totalMessages > -1) {
      this.info = {message: `${this.totalMessages} messages found for ${this.username}.`, severity: "info"}
    }
    if (this.totalMessages === 0) {
      this.info = {message: `No messages found for ${this.username}.`, severity: "warn"}
    }
